    ...
    class CommonOutputHandler < AssetAggregator::Core::OutputHandler
      ...
      def start_all
        ...
        when nil, :none, false then
          # nothing
        when :brief then
          output "/* '#{subpath}.#{extension}' @ #{mtime.to_i} */"
        when :full then
          output <<-END
/************************************************************************
 * '#{subpath}.#{extension}'
...
END
      ...
      def start_aggregator(aggregator)
        ...
        when nil, :none, false then
          # nothing
        when :brief then
          output "/* #{aggregator} @ #{aggregator.max_mtime_for(subpath)} */"
        when :encrypted then
          output "/* #{encrypt("#{aggregator} @ #{aggregator.max_mtime_for(subpath)}")} */"
        when :full then
          output <<-END


/************************************************************************
 * #{aggregator} (last modified: #{Time.at(aggregator.max_mtime_for(subpath))})
 ************************************************************************/
...
END
      ...
      def start_fragment(aggregator, fragment)
        ...
        when nil, :none, false then
          # nothing
        when :brief then
          output "/* #{fragment.source_position} @ #{fragment.mtime} */"
        when :encrypted then
          output "/* #{encrypt("#{fragment.source_position} @ #{fragment.mtime}")} */"
        when :full then
          output <<-END
/* ----------------------------------------------------------------------
   - #{fragment.source_position} (last modified: #{Time.at(fragment.mtime)})
   ---------------------------------------------------------------------- */
END
        else
