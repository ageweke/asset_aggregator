    ...
    class CommonOutputHandler < AssetAggregator::Core::OutputHandler
      ...
      def start_all
        ...
        when nil, :none, false then
          # nothing
        when :brief then
          output "/* '#{subpath}.#{extension}' @ #{short_time(mtime)} */"
        when :full then
          output <<-END
/************************************************************************
 * '#{subpath}.#{extension}'
...
END
      ...
      def start_aggregator(aggregator)
        ...
        when nil, :none, false then
          # nothing
        when :brief then
          output "/* #{aggregator} @ #{short_time(aggregator.max_mtime_for(subpath))} */"
        when :encrypted then
          output "/* #{encrypt("#{aggregator} @ #{short_time(aggregator.max_mtime_for(subpath))}")} */"
        when :full then
          output <<-END


/************************************************************************
 * #{aggregator} (last modified: #{long_time(aggregator.max_mtime_for(subpath))})
 ************************************************************************/
