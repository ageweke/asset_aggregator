...
# output from the #JavascriptOutputHandler. Rather, it uses regular expressions
...
describe AssetAggregator::OutputHandlers::JavascriptOutputHandler do
  before :each do
    ...
    @subpath = "foo/bar"
    @output_handler = AssetAggregator::OutputHandlers::JavascriptOutputHandler.new(@aggregate_type, @subpath)
  ...
  it "should put headers overall, per-aggregator, and per-fragment" do
    ...
    normalized_text.should match(%r{foo/bar.js.*aggregator1yo.*sp1.*yo ho ho.*sp2.*and a bottle of rum.*aggregator2yo.*sp3.*a pirate's life for me})
  end
end
